...
#' @return a fitted nlme model
#'
fitModel <- function(gDat, vStart = c(8.886464, 1.495953 ), bLargeScale=TRUE, bSilent=TRUE, P = 2){
  ...
  if(bLargeScale){
      if (P == 2){
            fmv5 <- nlme::nlme(y ~ logist3(x, xmid, scal),
                              ...
                              control = nlme::nlmeControl(returnObject = T),
                              )
            if(!bSilent){
              summary(fmv5)
            }
          ...
          {
          vStart = c(1.0, 8.886464, 1.495953 )
          fmv5 <- nlme::nlme(y ~ logist3P(x, B, xmid, scal),
                           fixed= list(xmid+scal~1, B~1),
                           random=list(CL = nlme::pdSymm(B+xmid+scal~1), 
                                       drug = nlme::pdDiag(xmid~1)),
                           data = gDat, start=vStart, method='REML')
        if(!bSilent){
          summary(fmv5)
        ...
        return(fmv5)
          ...
          {
          vStart = c(1.0, 1.0, 8.886464, 1.495953 )
          fmv5 <- nlme::nlme(y ~ logist4P(x,A,B, xmid, scal),
                           fixed= list(xmid+scal~1, B~1, A~1),
                           random=list(CL = nlme::pdSymm(A+B+xmid+scal~1), 
                                       drug = nlme::pdDiag(xmid~1)),
                           data = gDat, start=vStart, method='REML')
        if(!bSilent){
          summary(fmv5)
...
logist3P <- stats::selfStart( ~ B/(1 + exp(-(x - xmid)/scal)),
                             initial = function(mCall, LHS, data){   
                               xy <- stats::sortedXyData(mCall[["x"]], LHS, data)
                               if(nrow(xy) < 3) {
                                 stop("Too few distinct input values to fit a logistic")
                               ...
                               B <- 0.99
                               value <- c(B, xmid, scal)
                               names(value) <- mCall[c("B","xmid", "scal")]
                               value
                             },
                             parameters = c("B", "xmid", "scal"))

logist4P <- stats::selfStart( ~ A + (B - A)/(1 + exp(-(x - xmid)/scal)),
                             initial = function(mCall, LHS, data){   
                               xy <- stats::sortedXyData(mCall[["x"]], LHS, data)
                               if(nrow(xy) < 3) {
                                 stop("Too few distinct input values to fit a logistic")
